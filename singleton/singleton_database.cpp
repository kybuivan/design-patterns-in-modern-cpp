...
private:
    ...
    Database(const std::string& name, int version)
...
public:
    ...
    {
        std::cout << "Executing SQL query: " << sql << " on database: " << name << " version: " << version << std::endl;
    }

    ~Database()
    {
        std::cout << "Destroying database instance: " << name << " version: " << version << std::endl;
    }
};
